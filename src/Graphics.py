...
class Button:
    def __init__(self, y: int, x: int, h: int, w: int, color: tuple):
        self.y = y
        self.x = x
        self.h = h
        self.w = w
...
def draw_main_screen(sc, score: int, last: int, button: Button):
    sc.fill([255, 255, 255])
    button.draw(sc)
...
def draw_game_screen(sc, pl, balls, score: int):
    sc.fill([255, 255, 255])
